...
// ReverseReconciler reconciles a Reverse object
type ReverseReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=ip.ovh.kubeform.com,resources=reverses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ip.ovh.kubeform.com,resources=reverses/status,verbs=get;update;patch

func (r *ReverseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("reverse", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Reverse")
...
func (r *ReverseReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &ipv1alpha1.Reverse{}); err != nil {
			klog.Error(err, "unable to set up auditor", ipv1alpha1.Reverse{}.APIVersion, ipv1alpha1.Reverse{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&ipv1alpha1.Reverse{}).
