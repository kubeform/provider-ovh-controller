...
// IdentityUserReconciler reconciles a IdentityUser object
type IdentityUserReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=me.ovh.kubeform.com,resources=identityusers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=me.ovh.kubeform.com,resources=identityusers/status,verbs=get;update;patch

func (r *IdentityUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("identityuser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IdentityUser")
...
func (r *IdentityUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mev1alpha1.IdentityUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", mev1alpha1.IdentityUser{}.APIVersion, mev1alpha1.IdentityUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mev1alpha1.IdentityUser{}).
...
func (r *IdentityUserReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for IdentityUser")
			return nil
		}
		...
		resourceList := &mev1alpha1.IdentityUserList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list IdentityUser")
			return nil
		}
