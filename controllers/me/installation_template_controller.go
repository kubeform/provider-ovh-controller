...
// InstallationTemplateReconciler reconciles a InstallationTemplate object
type InstallationTemplateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=me.ovh.kubeform.com,resources=installationtemplates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=me.ovh.kubeform.com,resources=installationtemplates/status,verbs=get;update;patch

func (r *InstallationTemplateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("installationtemplate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch InstallationTemplate")
...
func (r *InstallationTemplateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mev1alpha1.InstallationTemplate{}); err != nil {
			klog.Error(err, "unable to set up auditor", mev1alpha1.InstallationTemplate{}.APIVersion, mev1alpha1.InstallationTemplate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mev1alpha1.InstallationTemplate{}).
