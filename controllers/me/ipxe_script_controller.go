...
// IpxeScriptReconciler reconciles a IpxeScript object
type IpxeScriptReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=me.ovh.kubeform.com,resources=ipxescripts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=me.ovh.kubeform.com,resources=ipxescripts/status,verbs=get;update;patch

func (r *IpxeScriptReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipxescript", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpxeScript")
...
func (r *IpxeScriptReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &mev1alpha1.IpxeScript{}); err != nil {
			klog.Error(err, "unable to set up auditor", mev1alpha1.IpxeScript{}.APIVersion, mev1alpha1.IpxeScript{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&mev1alpha1.IpxeScript{}).
