...
// ServerRebootTaskReconciler reconciles a ServerRebootTask object
type ServerRebootTaskReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dedicated.ovh.kubeform.com,resources=serverreboottasks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dedicated.ovh.kubeform.com,resources=serverreboottasks/status,verbs=get;update;patch

func (r *ServerRebootTaskReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serverreboottask", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServerRebootTask")
...
func (r *ServerRebootTaskReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dedicatedv1alpha1.ServerRebootTask{}); err != nil {
			klog.Error(err, "unable to set up auditor", dedicatedv1alpha1.ServerRebootTask{}.APIVersion, dedicatedv1alpha1.ServerRebootTask{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dedicatedv1alpha1.ServerRebootTask{}).
