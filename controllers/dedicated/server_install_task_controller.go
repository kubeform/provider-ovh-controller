...
// ServerInstallTaskReconciler reconciles a ServerInstallTask object
type ServerInstallTaskReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dedicated.ovh.kubeform.com,resources=serverinstalltasks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dedicated.ovh.kubeform.com,resources=serverinstalltasks/status,verbs=get;update;patch

func (r *ServerInstallTaskReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serverinstalltask", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServerInstallTask")
...
func (r *ServerInstallTaskReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dedicatedv1alpha1.ServerInstallTask{}); err != nil {
			klog.Error(err, "unable to set up auditor", dedicatedv1alpha1.ServerInstallTask{}.APIVersion, dedicatedv1alpha1.ServerInstallTask{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dedicatedv1alpha1.ServerInstallTask{}).
