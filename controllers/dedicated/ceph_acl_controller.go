...
// CephACLReconciler reconciles a CephACL object
type CephACLReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dedicated.ovh.kubeform.com,resources=cephacls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dedicated.ovh.kubeform.com,resources=cephacls/status,verbs=get;update;patch

func (r *CephACLReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cephacl", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CephACL")
...
func (r *CephACLReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dedicatedv1alpha1.CephACL{}); err != nil {
			klog.Error(err, "unable to set up auditor", dedicatedv1alpha1.CephACL{}.APIVersion, dedicatedv1alpha1.CephACL{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dedicatedv1alpha1.CephACL{}).
