...
// ServerUpdateReconciler reconciles a ServerUpdate object
type ServerUpdateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dedicated.ovh.kubeform.com,resources=serverupdates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dedicated.ovh.kubeform.com,resources=serverupdates/status,verbs=get;update;patch

func (r *ServerUpdateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("serverupdate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServerUpdate")
...
func (r *ServerUpdateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dedicatedv1alpha1.ServerUpdate{}); err != nil {
			klog.Error(err, "unable to set up auditor", dedicatedv1alpha1.ServerUpdate{}.APIVersion, dedicatedv1alpha1.ServerUpdate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dedicatedv1alpha1.ServerUpdate{}).
