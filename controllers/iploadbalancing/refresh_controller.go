...
// RefreshReconciler reconciles a Refresh object
...
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=refreshes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=refreshes/status,verbs=get;update;patch

func (r *RefreshReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("refresh", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Refresh")
...
func (r *RefreshReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iploadbalancingv1alpha1.Refresh{}); err != nil {
			klog.Error(err, "unable to set up auditor", iploadbalancingv1alpha1.Refresh{}.APIVersion, iploadbalancingv1alpha1.Refresh{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iploadbalancingv1alpha1.Refresh{}).
