...
// IploadbalancingReconciler reconciles a Iploadbalancing object
type IploadbalancingReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=iploadbalancings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=iploadbalancings/status,verbs=get;update;patch

func (r *IploadbalancingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("iploadbalancing", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Iploadbalancing")
...
func (r *IploadbalancingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iploadbalancingv1alpha1.Iploadbalancing{}); err != nil {
			klog.Error(err, "unable to set up auditor", iploadbalancingv1alpha1.Iploadbalancing{}.APIVersion, iploadbalancingv1alpha1.Iploadbalancing{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iploadbalancingv1alpha1.Iploadbalancing{}).
...
func (r *IploadbalancingReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Iploadbalancing")
			return nil
		}
		...
		resourceList := &iploadbalancingv1alpha1.IploadbalancingList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Iploadbalancing")
			return nil
		}
