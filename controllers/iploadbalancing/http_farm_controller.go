...
// HttpFarmReconciler reconciles a HttpFarm object
type HttpFarmReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=httpfarms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=httpfarms/status,verbs=get;update;patch

func (r *HttpFarmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("httpfarm", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HttpFarm")
...
func (r *HttpFarmReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iploadbalancingv1alpha1.HttpFarm{}); err != nil {
			klog.Error(err, "unable to set up auditor", iploadbalancingv1alpha1.HttpFarm{}.APIVersion, iploadbalancingv1alpha1.HttpFarm{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iploadbalancingv1alpha1.HttpFarm{}).
