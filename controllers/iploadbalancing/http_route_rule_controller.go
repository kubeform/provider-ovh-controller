...
// HttpRouteRuleReconciler reconciles a HttpRouteRule object
type HttpRouteRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=httprouterules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=httprouterules/status,verbs=get;update;patch

func (r *HttpRouteRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("httprouterule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HttpRouteRule")
...
func (r *HttpRouteRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iploadbalancingv1alpha1.HttpRouteRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", iploadbalancingv1alpha1.HttpRouteRule{}.APIVersion, iploadbalancingv1alpha1.HttpRouteRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iploadbalancingv1alpha1.HttpRouteRule{}).
