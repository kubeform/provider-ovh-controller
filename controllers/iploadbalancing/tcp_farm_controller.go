...
// TcpFarmReconciler reconciles a TcpFarm object
type TcpFarmReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=tcpfarms,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=iploadbalancing.ovh.kubeform.com,resources=tcpfarms/status,verbs=get;update;patch

func (r *TcpFarmReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tcpfarm", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TcpFarm")
...
func (r *TcpFarmReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &iploadbalancingv1alpha1.TcpFarm{}); err != nil {
			klog.Error(err, "unable to set up auditor", iploadbalancingv1alpha1.TcpFarm{}.APIVersion, iploadbalancingv1alpha1.TcpFarm{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&iploadbalancingv1alpha1.TcpFarm{}).
