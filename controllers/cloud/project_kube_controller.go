...
// ProjectKubeReconciler reconciles a ProjectKube object
type ProjectKubeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.ovh.kubeform.com,resources=projectkubes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.ovh.kubeform.com,resources=projectkubes/status,verbs=get;update;patch

func (r *ProjectKubeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("projectkube", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProjectKube")
...
func (r *ProjectKubeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.ProjectKube{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.ProjectKube{}.APIVersion, cloudv1alpha1.ProjectKube{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.ProjectKube{}).
...
func (r *ProjectKubeReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ProjectKube")
			return nil
		}
		...
		resourceList := &cloudv1alpha1.ProjectKubeList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ProjectKube")
			return nil
		}
