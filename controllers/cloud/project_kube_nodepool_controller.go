...
// ProjectKubeNodepoolReconciler reconciles a ProjectKubeNodepool object
type ProjectKubeNodepoolReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.ovh.kubeform.com,resources=projectkubenodepools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.ovh.kubeform.com,resources=projectkubenodepools/status,verbs=get;update;patch

func (r *ProjectKubeNodepoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("projectkubenodepool", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProjectKubeNodepool")
...
func (r *ProjectKubeNodepoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.ProjectKubeNodepool{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.ProjectKubeNodepool{}.APIVersion, cloudv1alpha1.ProjectKubeNodepool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.ProjectKubeNodepool{}).
