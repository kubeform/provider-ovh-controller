...
// ProjectContainerregistryUserReconciler reconciles a ProjectContainerregistryUser object
type ProjectContainerregistryUserReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.ovh.kubeform.com,resources=projectcontainerregistryusers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.ovh.kubeform.com,resources=projectcontainerregistryusers/status,verbs=get;update;patch

func (r *ProjectContainerregistryUserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("projectcontainerregistryuser", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProjectContainerregistryUser")
...
func (r *ProjectContainerregistryUserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.ProjectContainerregistryUser{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.ProjectContainerregistryUser{}.APIVersion, cloudv1alpha1.ProjectContainerregistryUser{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.ProjectContainerregistryUser{}).
...
func (r *ProjectContainerregistryUserReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ProjectContainerregistryUser")
			return nil
		}
		...
		resourceList := &cloudv1alpha1.ProjectContainerregistryUserList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ProjectContainerregistryUser")
			return nil
		}
