...
// ProjectNetworkPrivateReconciler reconciles a ProjectNetworkPrivate object
type ProjectNetworkPrivateReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=cloud.ovh.kubeform.com,resources=projectnetworkprivates,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=cloud.ovh.kubeform.com,resources=projectnetworkprivates/status,verbs=get;update;patch

func (r *ProjectNetworkPrivateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("projectnetworkprivate", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProjectNetworkPrivate")
...
func (r *ProjectNetworkPrivateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &cloudv1alpha1.ProjectNetworkPrivate{}); err != nil {
			klog.Error(err, "unable to set up auditor", cloudv1alpha1.ProjectNetworkPrivate{}.APIVersion, cloudv1alpha1.ProjectNetworkPrivate{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&cloudv1alpha1.ProjectNetworkPrivate{}).
