...
// LogsOutputGraylogStreamReconciler reconciles a LogsOutputGraylogStream object
type LogsOutputGraylogStreamReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dbaas.ovh.kubeform.com,resources=logsoutputgraylogstreams,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dbaas.ovh.kubeform.com,resources=logsoutputgraylogstreams/status,verbs=get;update;patch

func (r *LogsOutputGraylogStreamReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("logsoutputgraylogstream", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LogsOutputGraylogStream")
...
func (r *LogsOutputGraylogStreamReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dbaasv1alpha1.LogsOutputGraylogStream{}); err != nil {
			klog.Error(err, "unable to set up auditor", dbaasv1alpha1.LogsOutputGraylogStream{}.APIVersion, dbaasv1alpha1.LogsOutputGraylogStream{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dbaasv1alpha1.LogsOutputGraylogStream{}).
