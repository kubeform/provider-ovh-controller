...
// LogsInputReconciler reconciles a LogsInput object
type LogsInputReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dbaas.ovh.kubeform.com,resources=logsinputs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dbaas.ovh.kubeform.com,resources=logsinputs/status,verbs=get;update;patch

func (r *LogsInputReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("logsinput", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LogsInput")
...
func (r *LogsInputReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dbaasv1alpha1.LogsInput{}); err != nil {
			klog.Error(err, "unable to set up auditor", dbaasv1alpha1.LogsInput{}.APIVersion, dbaasv1alpha1.LogsInput{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dbaasv1alpha1.LogsInput{}).
...
func (r *LogsInputReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for LogsInput")
			return nil
		}
		...
		resourceList := &dbaasv1alpha1.LogsInputList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list LogsInput")
			return nil
		}
