...
// +kubebuilder:rbac:groups=vrack.ovh.kubeform.com,resources=cloudprojects,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vrack.ovh.kubeform.com,resources=cloudprojects/status,verbs=get;update;patch

func (r *CloudprojectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("cloudproject", req.NamespacedName)
...
func (r *CloudprojectReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vrackv1alpha1.Cloudproject{}); err != nil {
			klog.Error(err, "unable to set up auditor", vrackv1alpha1.Cloudproject{}.APIVersion, vrackv1alpha1.Cloudproject{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vrackv1alpha1.Cloudproject{}).
