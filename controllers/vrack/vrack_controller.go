...
// VrackReconciler reconciles a Vrack object
...
// +kubebuilder:rbac:groups=vrack.ovh.kubeform.com,resources=vracks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vrack.ovh.kubeform.com,resources=vracks/status,verbs=get;update;patch

func (r *VrackReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vrack", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Vrack")
...
func (r *VrackReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vrackv1alpha1.Vrack{}); err != nil {
			klog.Error(err, "unable to set up auditor", vrackv1alpha1.Vrack{}.APIVersion, vrackv1alpha1.Vrack{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vrackv1alpha1.Vrack{}).
