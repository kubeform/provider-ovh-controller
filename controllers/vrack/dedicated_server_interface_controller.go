...
// DedicatedServerInterfaceReconciler reconciles a DedicatedServerInterface object
type DedicatedServerInterfaceReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=vrack.ovh.kubeform.com,resources=dedicatedserverinterfaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vrack.ovh.kubeform.com,resources=dedicatedserverinterfaces/status,verbs=get;update;patch

func (r *DedicatedServerInterfaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dedicatedserverinterface", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DedicatedServerInterface")
...
func (r *DedicatedServerInterfaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vrackv1alpha1.DedicatedServerInterface{}); err != nil {
			klog.Error(err, "unable to set up auditor", vrackv1alpha1.DedicatedServerInterface{}.APIVersion, vrackv1alpha1.DedicatedServerInterface{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vrackv1alpha1.DedicatedServerInterface{}).
