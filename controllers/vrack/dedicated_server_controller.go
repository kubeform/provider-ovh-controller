...
// +kubebuilder:rbac:groups=vrack.ovh.kubeform.com,resources=dedicatedservers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vrack.ovh.kubeform.com,resources=dedicatedservers/status,verbs=get;update;patch

func (r *DedicatedServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dedicatedserver", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DedicatedServer")
...
func (r *DedicatedServerReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vrackv1alpha1.DedicatedServer{}); err != nil {
			klog.Error(err, "unable to set up auditor", vrackv1alpha1.DedicatedServer{}.APIVersion, vrackv1alpha1.DedicatedServer{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vrackv1alpha1.DedicatedServer{}).
