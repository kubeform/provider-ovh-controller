...
// ZoneRedirectionReconciler reconciles a ZoneRedirection object
type ZoneRedirectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=domain.ovh.kubeform.com,resources=zoneredirections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=domain.ovh.kubeform.com,resources=zoneredirections/status,verbs=get;update;patch

func (r *ZoneRedirectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("zoneredirection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ZoneRedirection")
...
func (r *ZoneRedirectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &domainv1alpha1.ZoneRedirection{}); err != nil {
			klog.Error(err, "unable to set up auditor", domainv1alpha1.ZoneRedirection{}.APIVersion, domainv1alpha1.ZoneRedirection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&domainv1alpha1.ZoneRedirection{}).
